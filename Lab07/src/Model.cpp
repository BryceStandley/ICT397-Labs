...
void Model::Draw(Shader& shader, glm::mat4 model)
{
    ...
    {
        glm::mat4 newModel = model * meshe.transform;
...
void Model::ProcessNode(aiNode* node, const aiScene* scene, glm::mat4 transform)// glm trasform
{
...
std::vector<Texture> Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
    TextureLoader tl;
    std::vector<Texture> textures;
    int count = (int)mat->GetTextureCount(type);
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {   // if texture hasn't been loaded already, load it
            ...
            texture.id = tl.TextureFromFile(filepath);
